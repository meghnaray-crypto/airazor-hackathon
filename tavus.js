...
(() => {
  ...
  async function startConversation(testMode = false) {
    ...
    try {
      ...
      const result = await api("/api/tavus/start", { test_mode: testMode, conversation_name: testMode ? "AIRazor connectivity test" : "AIRazor merchant demo" });
      if (testMode) {
  ...
  async function endConversation() {
    ...
    try { els.end.disabled = true; await api("/api/tavus/end", {conversation_id: currentConversationId}); }
      ...
      currentConversationId = null; currentConversationUrl = null; els.frame.src = "about:blank";
      els.live.classList.add("hidden"); els.empty.classList.remove("hidden"); els.end.disabled = false; setStatus("Tavus ready", "good");
    }
  }
